        ...
        <tr>
          ...
          <td class="am-display" rowspan="2" valign="top">
						...
						// Attempt to execute the prepared statement
							...
							$sql = "SELECT count(docu_id) AS docuCount FROM recipient WHERE reci_id=$id AND status=2";
							if($stmt = mysqli_prepare($link, $sql)){
					      // Attempt to execute the prepared statement
					      if(mysqli_stmt_execute($stmt)){
					          $result = mysqli_stmt_get_result($stmt);
					    ...
					    mysqli_stmt_close($stmt);
							...
							?>
            <h3 class="nav-header">all incoming documents<span class="badge badge-light"><?php echo $docuCount; ?></span></h3>
              <div class="menu-display" style="height: 350px">
								...
								<table class="user-list" style="width: 100%">
									...
									$sql = "SELECT * from document LEFT JOIN recipient ON document.docu_id=recipient.docu_id WHERE recipient.status=2 AND recipient.reci_id=$id ORDER BY document.docu_id DESC";
											...
											if(mysqli_num_rows($result) > 0) {
													while($row = mysqli_fetch_array($result)){
																	...
																	echo "<td class='action-icons'>
																			<a href='viewdocu.php?docu_id=". $row['docu_id'] ."&from=1'><img src='../img/view-icon.png' title='View'></a>
																			</td>";
																echo "</tr>";
															}
